...
export class MenuComponent extends InjectBase implements OnInit{
  ...
  ngOnInit(): void {
    this.contextItems = [
      { label: 'Thêm mới', icon: 'pi pi-plus', command: () => this.addNode() },
      { label: 'Chỉnh sửa', icon: 'pi pi-pencil', command: () => this.editNode() },
      { label: 'Xóa', icon: 'pi pi-trash', command: () => this.removeNode() },
      { separator: true },
      { label: 'Bỏ chọn', icon: 'pi pi-times', command: () => this.unselectFile() }
    ];
  ...
  getListController() {
    this._menuService.getListController()
      .subscribe({
        next: (res) => {
  ...
  getIcons() {
    this._iconService.getIcons().subscribe(data => {
      ...
      this.icons = icons.map(icon => {
        return { key: 'pi-' + icon.properties.name, value: 'pi-' + icon.properties.name }
      });
    });
  }
  //#endregion
  ...
  onControllerValueChange(event: KeyValuePair) {
    ...
    if (event) {
      ...
      this.menuItem.routerLink = `admin/${event.key.replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase()}`;
    }
  }
  ...
  onIconValueChange(event: KeyValuePair) {
    this.menuItem.icon = '';
    if (event)
      this.menuItem.icon = 'pi ' + event.key;
  }
  //#endregion
  ...
  saveDragDropable() {
    this._toast.confirm('Are you sure?', MessageConstants.CONFIRM_SAVE_MSG, () => this.acceptSaveDragDropable(), () => this.cancelDragDropable());
  }

  acceptSaveDragDropable() {
    this._progress.start();

    this._menuService.configurationMenus(this.nodes)
      .subscribe({
        next: (res: OperationResult) => {
          if (res.isSuccess) {
            this._toast.success(CaptionConstants.SUCCESS, 'Sắp xếp menus thành công !');
            this.dragdropable = false;
            this.loadMenus();
          }
          else
            this._toast.error(CaptionConstants.ERROR, res.message ? res.message : 'Sắp xếp menus có lỗi khi lưu !');
        },
        error: () => {
  ...
  addNode() {
    ...
    if (this.selectedFile && Object.keys(this.selectedFile).length > 0) {
      this.menuItem = <Menu>{
        ...
        seq: this.selectedFile.children?.length > 0 ? Math.max(...this.selectedFile.children.map(x => x.data.seq)) + 1 : 1,
        visible: true,
        separator: false
      }
    }
  ...
  editNode() {
    ...
    this.controllerSelect = this.controllers.find(x => x.key == this.menuItem.controller);
    this.iconSelect = this.icons.find(x => 'pi ' + x.key == this.menuItem.icon);
  ...
  //#region remove node
  removeNode() {
    this._toast.confirm('Are you sure?', MessageConstants.CONFIRM_DELETE_MSG, () => this.acceptDelete());
  }
  //#endregion
  ...
  save(menuForm: NgForm) {
    ...
    let success = !this.isEdit ? 'Thêm mới menu thành công !' : 'Chỉnh sửa menu thành công !';
    let error = !this.isEdit ? 'Thêm mới menu có lỗi khi lưu !' : 'Chỉnh sửa menu có lỗi khi lưu !';
      ...
      .subscribe({
        next: (res: OperationResult) => {
          if (res.isSuccess) {
            ...
            this._toast.success(CaptionConstants.ERROR, error);
        },
        error: () => {
          this._toast.error(CaptionConstants.ERROR, MessageConstants.UN_KNOWN_ERROR);
        }
  ...
  acceptDelete() {
    ...
    this._menuService.delete(this.selectedFile.data.id)
      .subscribe({
        next: (res: OperationResult) => {
          if (res.isSuccess) {
            this._toast.success(CaptionConstants.SUCCESS, `Xóa menu thành công !`);
            this.loadMenus();
          }
          else
            this._toast.error(CaptionConstants.ERROR, res.message ? res.message : `Xóa menu có lỗi khi lưu !`);
        },
        error: () => {
          this._toast.error(CaptionConstants.ERROR, MessageConstants.UN_KNOWN_ERROR);
        }
